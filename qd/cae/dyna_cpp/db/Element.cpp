 ...
 * Constructor.
 */
Element::Element(const int _elementID, const Element::ElementType _elementType,
                 const vector<size_t> &_node_indexes, DB_Elements *_db_elements)
    : is_rigid(false),
 ...
 */
void Element::set_is_rigid(bool _is_rigid) { this->is_rigid = _is_rigid; }

/** Get whether the element is rigid or not
 *
 * @return is_rigid rigid status of the element
 */
bool Element::get_is_rigid() const { return this->is_rigid; }
 ...
 * SOLID = 3
 */
Element::ElementType Element::get_elementType() const { return this->elemType; }
 ...
 */
vector<size_t> Element::get_node_indexes() const { return this->nodes; }
 ...
 */
vector<float> Element::get_energy() const { return this->energy; }
...
float Element::get_estimated_element_size() const {
  ...
  vector<float> basis_coords =
      db_nodes->get_nodeByIndex(this->nodes[0])->get_coords();
#endif
 ...
 */
vector<vector<float>> Element::get_strain() const { return this->strain; }
 ...
 */
vector<vector<float>> Element::get_stress() const { return this->stress; }

/** Get the mises stress over time
 *
 */
vector<float> Element::get_stress_mises() const { return this->stress_mises; }
 ...
 */
void Element::clear_energy() { this->energy.clear(); }

/** Clear the elements plastic strain
 */
void Element::clear_plastic_strain() { this->plastic_strain.clear(); }

/** Clear the elements stress
 */
void Element::clear_stress() { this->stress.clear(); }

/** Clear the elements mises stress
 */
void Element::clear_stress_mises() { this->stress_mises.clear(); }

/** Clear the elements strain
 */
void Element::clear_strain() { this->strain.clear(); }

/** Clear the elements history data
 */
void Element::clear_history_vars() { this->history_vars.clear(); }