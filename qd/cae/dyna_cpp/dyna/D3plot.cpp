...
/** Save the loaded data of the d3plot into a sql database
 * @param filepath : path for the saved file
 */
/*
void
D3plot::save_sqlite(const std::string& filepath, int32_t iTimestep)
{

 // python timestep style (negative means backwards)
 if (iTimestep < 0)
   iTimestep = static_cast<int32_t>(this->get_nTimesteps()) + iTimestep;
 if (iTimestep < 0)
   throw(
     std::invalid_argument("Specified timestep exceeds real time step size."));

 if (iTimestep >= static_cast<int32_t>(this->get_nTimesteps()))
   throw(
     std::invalid_argument("Specified timestep exceeds real time step size."));
 ...
 // open
 sqlite3* db = sqlite::open_database(filepath);

 // Create table: SHELLS
 // must be SAME ORDER as in the header of D3plot.hpp
 std::vector<std::string> shell_variables_to_write;
 std::vector<std::string> shell_variables_to_create;

 shell_variables_to_write.push_back("@id");
 shell_variables_to_write.push_back("@node_id_0");
 shell_variables_to_write.push_back("@node_id_1");
 shell_variables_to_write.push_back("@node_id_2");
 shell_variables_to_write.push_back("@node_id_3");

 shell_variables_to_create.push_back("id INT PRIMARY KEY NOT NULL");
 shell_variables_to_create.push_back("node_id_0 INT");
 shell_variables_to_create.push_back("node_id_1 INT");
 shell_variables_to_create.push_back("node_id_2 INT");
 shell_variables_to_create.push_back("node_id_3 INT");

 if (this->plastic_strain_is_read) {
   shell_variables_to_write.push_back("@plastic_strain");
   shell_variables_to_create.push_back("plastic_strain REAL");
 }
 if (this->energy_is_read) {
   shell_variables_to_write.push_back("@energy");
   shell_variables_to_create.push_back("energy REAL");
 }
 if (this->strain_is_read) {
   shell_variables_to_write.push_back("@strain_xx");
   shell_variables_to_write.push_back("@strain_xy");
   shell_variables_to_write.push_back("@strain_xz");
   shell_variables_to_write.push_back("@strain_yy");
   shell_variables_to_write.push_back("@strain_yz");
   shell_variables_to_write.push_back("@strain_xz");
   shell_variables_to_create.push_back("strain_xx REAL");
   shell_variables_to_create.push_back("strain_yy REAL");
   shell_variables_to_create.push_back("strain_zz REAL");
   shell_variables_to_create.push_back("strain_xy REAL");
   shell_variables_to_create.push_back("strain_yz REAL");
   shell_variables_to_create.push_back("strain_xz REAL");
 }
 if (this->stress_is_read) {
   shell_variables_to_write.push_back("@stress_xx");
   shell_variables_to_write.push_back("@stress_yy");
   shell_variables_to_write.push_back("@stress_zz");
   shell_variables_to_write.push_back("@stress_xy");
   shell_variables_to_write.push_back("@stress_yz");
   shell_variables_to_write.push_back("@stress_xz");
   shell_variables_to_create.push_back("stress_xx REAL");
   shell_variables_to_create.push_back("stress_yy REAL");
   shell_variables_to_create.push_back("stress_zz REAL");
   shell_variables_to_create.push_back("stress_xy REAL");
   shell_variables_to_create.push_back("stress_yz REAL");
   shell_variables_to_create.push_back("stress_xz REAL");
 }
 if (this->stress_mises_is_read) {
   shell_variables_to_write.push_back("@stress_mises");
   shell_variables_to_create.push_back("stress_mises REAL");
 }
 for (size_t iVar = 0; iVar < history_shell_is_read.size(); ++iVar) {
   shell_variables_to_write.push_back("@history_variable_" +
                                      std::to_string(iVar));
   shell_variables_to_create.push_back("history_variable_" +
                                       std::to_string(iVar) + " REAL");
 }

 // create table
 sqlite::create_table(db, "element_shell", shell_variables_to_create);

 // prepare for insertion (we want to do it fast!)
 sqlite3_stmt* sql_statement =
   sqlite::prepare_insertion(db, "element_shell", shell_variables_to_write);

 // start insertion/transaction
 sqlite::begin_insertion(db);

 for (size_t iElement = 0; iElement < db_elems->get_nElements(Element::SHELL);
      ++iElement) {

   int32_t iVar = 1;
   auto element = db_elems->get_elementByIndex(Element::SHELL, iElement);
   auto nodes = element->get_nodes();

   // insert values
   sqlite3_bind_int(sql_statement, iVar++, element->get_elementID());
   sqlite3_bind_int(sql_statement, iVar++, nodes[0]->get_nodeID());
   sqlite3_bind_int(sql_statement, iVar++, nodes[1]->get_nodeID());
   sqlite3_bind_int(sql_statement, iVar++, nodes[2]->get_nodeID());
   if (nodes.size() == 4) {
     sqlite3_bind_int(sql_statement, iVar++, nodes[3]->get_nodeID());
   } else {
     sqlite3_bind_int(sql_statement, iVar++, nodes[2]->get_nodeID());
   }
   if (this->plastic_strain_is_read)
     sqlite3_bind_double(
       sql_statement, iVar++, element->get_plastic_strain()[iTimestep]);
   if (this->energy_is_read)
     sqlite3_bind_double(
       sql_statement, iVar++, element->get_energy()[iTimestep]);
   if (this->strain_is_read) {
     auto strain = element->get_strain()[iTimestep];
     sqlite3_bind_double(sql_statement, iVar++, strain[0]);
     sqlite3_bind_double(sql_statement, iVar++, strain[1]);
     sqlite3_bind_double(sql_statement, iVar++, strain[2]);
     sqlite3_bind_double(sql_statement, iVar++, strain[3]);
     sqlite3_bind_double(sql_statement, iVar++, strain[4]);
     sqlite3_bind_double(sql_statement, iVar++, strain[5]);
   }
   if (this->stress_is_read) {
     auto stress = element->get_stress()[iTimestep];
     sqlite3_bind_double(sql_statement, iVar++, stress[0]);
     sqlite3_bind_double(sql_statement, iVar++, stress[1]);
     sqlite3_bind_double(sql_statement, iVar++, stress[2]);
     sqlite3_bind_double(sql_statement, iVar++, stress[3]);
     sqlite3_bind_double(sql_statement, iVar++, stress[4]);
     sqlite3_bind_double(sql_statement, iVar++, stress[5]);
   }
   if (this->stress_mises_is_read)
     sqlite3_bind_double(
       sql_statement, iVar++, element->get_stress_mises()[iTimestep]);

//if (history_shell_is_read) {
//  for (auto& entry : element->get_history_vars()) {
//    sqlite3_bind_double(sql_statement, iVar++, entry);
//  }
//}


   // buffer
   sqlite::perform_step(db, sql_statement);

   // reset bindings
   sqlite3_reset(sql_statement);
 }

 // end insertion/transaction
 sqlite::end_insertion(db, sql_statement);

 // close
 sqlite::close_database(db, sql_statement);
}
*/
