...
RawD3plot::read_airbag_section()
{
  ...
  if ((dyna_npefg > 0) && (dyna_npefg < 10000000)) {
    ...
    auto tensor_nlist = Tensor<int32_t>();
...
RawD3plot::read_geometry_elem8()
{
  ...
  // allocate ids
  auto elem8_nodes = Tensor<int32_t>();
...
RawD3plot::read_geometry_elem4()
{
  ...
  // allocate
  auto elem4_nodes = Tensor<int32_t>();
...
RawD3plot::read_geometry_elem2()
{
  ...
  // allocate
  auto elem2_nodes = Tensor<int32_t>();
...
RawD3plot::read_geometry_elem4th()
{
  ...
  // allocate
  auto elem4th_nodes = Tensor<int32_t>();
...
RawD3plot::read_geometry_numbering()
{
  ...
  */
  ...
  if (dyna_nel8 > 0) {
    auto elem_solid_ids = Tensor<int32_t>();
  ...
  if (dyna_nel2 > 0) {
    auto elem_beam_ids = Tensor<int32_t>();
  ...
  if (dyna_nel4 > 0) {
    auto elem_shell_ids = Tensor<int32_t>();
  ...
  if (dyna_nelth > 0) {
    auto elem_tshell_ids = Tensor<int32_t>();
...
RawD3plot::read_states()
{
  ...
  auto timesteps_tensor = Tensor<float>();
...
RawD3plot::read_states_elem4()
{
  ...
  // allocate
  auto shell_layer_vars = Tensor<float>();
  float_data.insert(
...
RawD3plot::read_states_elem4th()
{
  ...
  // allocate
  auto tshell_layer_vars = Tensor<float>();
  float_data.insert(
...
RawD3plot::read_states_airbag()
{
  ...
  // allocate
  auto tensor_float = Tensor<float>();
  float_data.insert(
  ...
  auto tensor_int = Tensor<int32_t>();
  ...
  for (int32_t ii = 0; ii < wordsToRead; ++ii) {
    if (dyna_airbag_nlist[nlist_offset + ii % dyna_airbag_state_geom] == 1) {
      tensor_int_vec->operator[](int_offset + iIntegerVar) =
        this->buffer->read_int(start + ii);
      ++iIntegerVar;
    } else {
      tensor_float_vec->operator[](float_offset + iFloatVar) =
  ...
  // allocate
  auto tensor_float2 = Tensor<float>();
  float_data.insert(
  ...
  auto tensor_int2 = Tensor<int32_t>();
  ...
  for (int32_t ii = 0; ii < wordsToRead; ++ii) {
    if (dyna_airbag_nlist[dyna_airbag_ngeom + ii % dyna_airbag_state_nvars] ==
        1) {
      tensor_int2_vec->operator[](int_offset + iIntegerVar) =
        this->buffer->read_int(start + ii);
      ++iIntegerVar;
    } else {
      tensor_float_vec2->operator[](float_offset + iFloatVar) =
        this->buffer->read_float(start + ii);
      ++iFloatVar;
    }
  }
